    ...
    <nav className="gc-bottom-nav fixed z-50 mx-auto max-w-lg rounded-[20px] border border-white/10 bg-[#131620]/96 p-1.5 shadow-[0_20px_60px_rgba(0,0,0,.5)] backdrop-blur-2xl md:hidden" aria-label="Primary navigation">
      <div className="grid grid-cols-5 gap-1">
        {MAIN_NAVIGATION_ITEMS.map((item) => {
          ...
          const isTrain = item.id === "train";
          return (
            <Link
              key={item.id}
              href={item.href}
              aria-current={active ? "page" : undefined}
              className={`relative flex min-h-[3.45rem] flex-col items-center justify-center gap-1 rounded-[15px] px-1 text-[10px] font-semibold transition ${
                isTrain
                  ? "bg-indigo-300 text-[#11131a] shadow-lg shadow-indigo-500/10"
                  : active
                    ? "bg-white/[0.07] text-indigo-200"
                    : "text-neutral-500 hover:text-neutral-200"
              }`}
            >
              <Icon className={`h-5 w-5 ${active || isTrain ? "stroke-[2.4]" : ""}`} aria-hidden />

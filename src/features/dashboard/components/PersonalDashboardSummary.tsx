...
export function PersonalDashboardSummary({ userId }: { userId: UUID }) {
  ...
  const stats = [
    { label: "Weekly plan", value: formatAdherencePercentage(summary.adherence.weekly), detail: `${summary.adherence.weeklyCompleted}/${summary.adherence.weeklyScheduled} sessions`, icon: Target, tone: "text-lime-300" },
    ...
    <section className="gc-card p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-black">Your week at a glance</h2>
        </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
        {stats.map(({ label, value, detail, icon: Icon, tone }) => (
          <div key={label} className="gc-stat min-w-0">
            ...
            <strong className="mt-2 block truncate text-lg font-black sm:text-2xl">{value}</strong>
            <span className="mt-1 block truncate text-[10px] font-black uppercase tracking-[0.08em] text-neutral-500 sm:text-[11px]">{label}</span>

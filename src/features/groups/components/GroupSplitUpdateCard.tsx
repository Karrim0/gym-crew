...
export function GroupSplitUpdateCard({ userId }: { userId: UUID }) {
  ...
  return (
    <section className="rounded-[26px] border border-amber-400/50 bg-amber-50 p-4 dark:bg-amber-950/20">
      <div className="flex gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-400 text-neutral-950"><RefreshCw className="h-5 w-5" /></span><div><h3 className="font-black">Group split was updated</h3><p className="mt-1 text-sm text-neutral-600 dark:text-neutral-300">Choose whether to copy it into your personal plan. Your current plan will never change silently.</p></div></div>
      <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" disabled={busy !== null} onClick={() => void keepPersonal()} className="rounded-xl border px-3 py-3 text-sm font-black">Keep mine</button><button type="button" disabled={busy !== null} onClick={() => void applyGroup()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-neutral-950 px-3 py-3 text-sm font-black text-white dark:bg-white dark:text-neutral-950"><ShieldCheck className="h-4 w-4" />Use group plan</button></div>
    </section>
  );
}

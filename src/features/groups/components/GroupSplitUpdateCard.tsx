...
export function GroupSplitUpdateCard({ userId }: { userId: UUID }) {
  ...
  return (
    <section className="rounded-[26px] border border-amber-400/50 bg-amber-50 p-4 dark:bg-amber-950/20">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" disabled={busy !== null} onClick={() => void keepPersonal()} className="rounded-xl border px-3 py-3 text-sm font-black">Keep mine</button><button type="button" disabled={busy !== null} onClick={() => void applyGroup()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-lime-300 px-3 py-3 text-sm font-black text-neutral-950"><ShieldCheck className="h-4 w-4" />Use group plan</button></div>
    </section>
  );
}
